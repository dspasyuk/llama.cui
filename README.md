...
#### Change  "--n-gpu-layers" in config.js file depending on the type of architecture used and available VRAM. For the default model (Llama3-instruct) this should be equal to 35, for safety it is currently set to 25, you will need at least 6Gb of Vram to run the model, so Nvidia GTX1060 and above is a must.  
